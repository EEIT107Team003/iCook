	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light">
		...
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<form class="form-inline my-2 my-lg-0">
				...
				<button class="btn btn-outline-success my-2 my-sm-0" type="submit" formaction="search" formaction="search?title=${ param.title }">Search</button>
			</form>
			...
			<div class="row">
				...
				<div class="col-8 table-responsive">
					<table class="table table-bordered table-hover">
						<th class="table-success" >標題</th>
						<th class="table-success">作者</th>
						<th class="table-success">內文</th>
						<th class="table-success">最後編輯時間</th>
						<c:forEach var="post" items="${ posts }">
							<tr>
								<td ><a
									href="pick?harticle_id=${ post.harticle_id }&article_id=${ post.article_id }">${ post.title }</a></td>
								<td>${ post.nickname }</td>
								<td>${ post.text }</td>
								<td>${ post.editTime }</td>
							</tr>
